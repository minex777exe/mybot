...
SERVER_COUNT_CHANNEL_ID = 1452948403783733248  # Replace with your server count channel ID
USER_COUNT_CHANNEL_ID = 1452948441134010472    # Replace with your user count channel ID
LOG_CHANNEL_ID = 1452948479323148299 # Replace with the channel ID for join/leave logs
...
async def on_guild_join(guild: discord.Guild):
    ...
    if log_channel:
        await log_channel.send("Fire Bot has been added to the server: **{guild.name}** (ID: `{guild.id}`)")
...
async def on_command_completion(context: commands.Context) -> None:
    ...
    async with aiohttp.ClientSession() as session:
        ...
        embed.set_footer(text="Frost Fire Development™ ❤️", icon_url=client.user.display_avatar.url)
